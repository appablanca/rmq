...
\begin{table}[h]
	...
	\begin{tabular}{lcc}
		...
		Blocks Precompute & $O\!\left(\frac{n}{S}\log^2 n\right)$ & $O(S)$$^{\ast}$ \\
			                                                                          Cartesian Trees   & $\tilde{O}(n\ logn)$ & $O(1)$ \\
		\hline
	\end{tabular}
...
\texttt{SegmentTree} (\texttt{SeTr}) stores, like the Sparse Table, indices of minima rather than the values themselves, but over intervals of powers of 2 instead of all overlapping ones.
...
and adds two arrays \texttt{prefixMin} and \texttt{suffixMin}, each a \texttt{std::vector<std::vector<uint32\_t>{}>} of length $n$.
...
Further optimisations can be done by using bitpacking for storing the values/indices and possibly utilizing parrellization on the memory accsesses using $SIMD$ like instructions,but the Apple M2 implements NEON without a gather instruction, so it is not available here.
